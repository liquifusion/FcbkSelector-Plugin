<cfcomponent output="false" mixin="controller">
	...
	<cffunction name="fcbkSelector" hint="Displays a Facebook-like multi-selector widget.">
		...
		<cfargument name="width" type="numeric" required="false" default="#application.fcbkSelector.width#" hint="Width of the widget.">
		<cfargument name="itemHeight" type="numeric" required="false" default="#application.fcbkSelector.itemHeight#" hint="Height of each item in the widget.">
		<cfargument name="itemsPerRow" type="numeric" required="false" default="#application.fcbkSelector.itemsPerRow#" hint="Number of items to display in each row.">
		<cfargument name="optionIdProperty" type="variablename" required="false" default="#application.fcbkSelector.optionIdProperty#" hint="Name of options' property containing the ID to associate with.">
		<cfargument name="optionTextProperty" type="variablename" required="false" default="#application.fcbkSelector.optionTextProperty#" hint="Name of options' property containing the text to display.">
		<cfargument name="parentPosInCompositeKey" type="numeric" required="false" default="1" hint="Parent object's position in the composite primary key in the database. This is dictated by column order in the database. If the parent is represented by the second column, for example, pass a value of `2`.">
		<cfargument name="legend" type="string" required="false" default="false" hint="Pass a string to display as the `<legend>` if you want for the selector to be wrapped in a `<fieldset>`. Pass `false` if you do not want for the selector to be wrapped in a `<fieldset>`.">
		<cfargument name="embedImplementationInHead" type="boolean" required="false" default="#application.fcbkSelector.embedImplementationInHtml#" hint="Whether or not to include the implementation in your HTML source inside of a <script> tag. Pass `false` if you plan on writing your own implementation script and implementing the jQuery call yourself.">
		<cfargument name="includeJQuery" type="boolean" required="false" default="#application.fcbkSelector.includeJQuery#" hint="Whether or not to include jQuery in your document's <head> automatically.">
		<cfargument name="includeJQueryPlugin" type="boolean" required="false" default="#application.fcbkSelector.includeJQueryPlugin#" hint="Whether or not to include the included fcbkListSelection jQuery plugin in your document's `<head>` automatically.">
		<cfargument name="includeStylesheet" type="boolean" required="false" default="#application.fcbkSelector.includeStylesheet#" hint="Whether or not to include our stylesheet in your document's <head> automatically.">
	...
	<cffunction name="$buildKeysString" access="private" returntype="string" hint="Builds string to pass as `hasManyCheckBox()`'s `keys` argument'.">
		<cfargument name="parentObject" hint="Object containing the parent represented by this selection.">
